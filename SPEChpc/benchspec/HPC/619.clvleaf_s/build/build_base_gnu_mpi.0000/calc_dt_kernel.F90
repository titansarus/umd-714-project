...
#ifdef SPEC_SPLIT_TARGET_REDUCTION
!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) &
!$OMP REDUCTION(MIN : dt_min_val) &
!$OMP MAP(tofrom:  &
!$OMP    celldx,celldy,cellx,celly,density0,soundspeed,viscosity_a,volume, &
!$OMP    xarea,xvel0,yarea,yvel0,dt_min,dt_min_val) &
!$OMP MAP(to:g_small)
#else
!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) &
!$OMP REDUCTION(MIN : dt_min_val) &
!$OMP MAP(tofrom:  &
!$OMP    celldx,celldy,cellx,celly,density0,soundspeed,viscosity_a,volume, &
!$OMP    xarea,xvel0,yarea,yvel0,dt_min) &
!$OMP MAP(to:g_small)
#endif
#endif

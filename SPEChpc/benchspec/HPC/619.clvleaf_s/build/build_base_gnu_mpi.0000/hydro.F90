...
SUBROUTINE hydro
  ...
  !$OMP TARGET DATA &
    ...
    !$OMP map(to:chunk%left_snd_buffer)    &
    !$OMP map(to:chunk%left_rcv_buffer)    &
    !$OMP map(to:chunk%right_snd_buffer)   &
    !$OMP map(to:chunk%right_rcv_buffer)   &
    !$OMP map(to:chunk%bottom_snd_buffer)  &
    !$OMP map(to:chunk%bottom_rcv_buffer)  &
    !$OMP map(to:chunk%top_snd_buffer)     &
    !$OMP map(to:chunk%top_rcv_buffer)
#endif
    DO

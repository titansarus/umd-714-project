...
// Default for OPENMP is to target host.  
//    Override by setting -DSPEC_USE_HOST_THREADS=0
// Default for MPI-only, OpenACC and OpenMP Target is SPEC_TARGET_DEFAULT
//    Override by setting -DSPEC_USE_HOST_THREADS=1
#ifndef SPEC_USE_HOST_THREADS
#  ifdef SPEC_OPENMP
#    define SPEC_USE_HOST_THREADS 1
#  else 
#    define SPEC_USE_HOST_THREADS 0
#  endif
#endif
                     ...
                     Arguments*        args )
{
  ...
  /*====================*/
#if (SPEC_USE_HOST_THREADS==1) 
  sweeper->nthread_octant = NOCTANT;
#else
...
#if defined(SWEEPER_KBA_ACC) || defined(SWEEPER_KBA_OPENMP_TARGET)
  {
    size_t v_size = Dimensions_size_state( sweeper->dims, NU );
    P* vo_h = Pointer_h( vo );
#   if defined(SWEEPER_KBA_ACC)
#   pragma acc parallel loop gang vector present(vo_h[:v_size])
#   elif defined(SWEEPER_KBA_OPENMP_TARGET)
...
#   endif
  ...
  for( step=0-1; step<nstep+1; ++step )
  {
    ...
    P* facexy_h = Pointer_h(facexy);
    P* facexz_h = Pointer_h(facexz);
    P* faceyz_h = Pointer_h(faceyz);
...
#pragma acc enter data                          \
  create(facexy_h[:facexy_size]),      \
  create(facexz_h[:facexz_size]),      \
  create(faceyz_h[:faceyz_size])
    }
#endif    
    ...
    #pragma acc exit data \
      delete(facexy_h[:facexy_size]),  \
      delete(facexz_h[:facexz_size]),  \
      delete(faceyz_h[:faceyz_size])      
    }
#endif    
...
/*===========================================================================*/
/*---Allocate and copyin device storage---*/

void Sweeper_alloc_and_copy_to_device(
  Sweeper*               sweeper,
  Pointer*               vo,
  Pointer*               vi,
  Quantities*            quan,
  Env*                   env )
{
# if defined(USE_OPENMP_TARGET) || defined(USE_ACC)
    P* vi_h       = Pointer_h( vi );              size_t v_size        = Pointer_n( vi );
    P* vo_h       = Pointer_h( vo );
    P* a_from_m_h = Pointer_h( &quan->a_from_m ); size_t a_from_m_size = Pointer_n( &quan->a_from_m );
    P* m_from_a_h = Pointer_h( &quan->m_from_a ); size_t m_from_a_size = Pointer_n( &quan->m_from_a );
    P* vs_local_h = sweeper->vslocal_host_;       size_t vs_local_size = Sweeper_nvslocal_( sweeper, env );
#   if defined(USE_OPENMP_TARGET)
#     pragma omp target enter data map(to: vi_h[0:v_size], \
                                           vo_h[0:v_size], \
                                           a_from_m_h[0:a_from_m_size], \
                                           m_from_a_h[0:m_from_a_size]), \
                                   map(alloc: vs_local_h[0:vs_local_size])
#   elif defined(USE_ACC)
#     pragma acc enter data copyin(vi_h[:v_size]), \
                            copyin(vo_h[:v_size]), \
                            copyin(a_from_m_h[:a_from_m_size]), \
                            copyin(m_from_a_h[:m_from_a_size]), \
                            create(vs_local_h[:vs_local_size])
#   endif
# endif
}

/*===========================================================================*/
/*---Copyout and free device storage---*/

void Sweeper_copy_to_host_and_free(
  Sweeper*               sweeper,
  Pointer*               vo,
  Pointer*               vi,
  Quantities*            quan,
...
# if defined(USE_OPENMP_TARGET) || defined(USE_ACC)
    ...
    P* vs_local_h = sweeper->vslocal_host_;       size_t vs_local_size = Sweeper_nvslocal_( sweeper, env );
#   if defined(USE_OPENMP_TARGET)
#     pragma omp target exit data map(from: vi_h[0:v_size], \
                                            vo_h[0:v_size]), \
                                  map(delete: a_from_m_h[0:a_from_m_size], \
                                              m_from_a_h[0:m_from_a_size], \
                                              vs_local_h[0:vs_local_size])
#   elif defined(USE_ACC)
#     pragma acc exit data copyout(vi_h[:v_size]), \
                           copyout(vo_h[:v_size]), \
                           delete(a_from_m_h[:a_from_m_size]), \
                           delete(m_from_a_h[:m_from_a_size]), \
                           delete(vs_local_h[:vs_local_size])
#   endif
# endif
}

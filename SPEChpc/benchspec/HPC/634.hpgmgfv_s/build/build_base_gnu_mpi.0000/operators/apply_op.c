...
void apply_op(level_type * level, int Ax_id, int x_id, double a, double b){
  ...
  if(level->use_offload)do_sync(); // FIX... wait for any other GPU operations on this level to complete

  // The code below is executed on the CPU and has the side effect of updating level vectors

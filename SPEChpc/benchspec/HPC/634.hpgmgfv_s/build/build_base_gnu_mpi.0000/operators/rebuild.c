...
double power_method(level_type * level, double a, double b, int max_iterations){
  ...
  for(i=0;i<max_iterations;i++){
   ...
   double Ax_max = norm2(level,Ax_id); // renormalize Ax (== new x)
...
void rebuild_operator_blackbox(level_type * level, double a, double b, int colors_in_each_dim){
  ...
  for(icolor=0;icolor<colors_in_each_dim;icolor++){
    ...
    if (level->use_offload) {
      device_rebuild(level, x_id, Aii_id, sumAbsAij_id, a, b);
    }
    else {
...
# if defined(SPEC_OPENACC)
    ...
    copy_one_vector_to_host(level, Aii_id);
    copy_one_vector_to_host(level, sumAbsAij_id);
# elif defined(SPEC_OPENMP_TARGET)
    // copy_vector_to_host_openmp(level);
    copy_one_vector_to_host_openmp(level, Aii_id);
    copy_one_vector_to_host_openmp(level, sumAbsAij_id);
# endif
  }
#endif
  ...
  if (level->use_offload) {
# if defined(SPEC_OPENACC)
    // copy_vector_to_device(level);
    copy_one_vector_to_device(level, Aii_id);
    copy_one_vector_to_device(level, sumAbsAij_id);
# elif defined(SPEC_OPENMP_TARGET)
    // copy_vector_to_device_openmp(level);
    copy_one_vector_to_device_openmp(level, Aii_id);
    copy_one_vector_to_device_openmp(level, sumAbsAij_id);
# endif
  }
#endif

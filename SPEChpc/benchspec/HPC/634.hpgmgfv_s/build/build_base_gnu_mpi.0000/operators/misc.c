...
void initialize_valid_region(level_type * level){
  ...
  if(level->use_offload)do_sync(); // FIX... no CUDA version... must sync CPU/GPU before using CPU version...
...
void add_vectors(level_type * level, int id_c, double scale_a, int id_a, double scale_b, int id_b){
  ...
  if (level->use_offload) {
    device_add_vectors(level, id_c, scale_a, id_a, scale_b, id_b);
  }
  else {
  PRAGMA_THREAD_ACROSS_BLOCKS(level,block,level->num_my_blocks)
  for(block=0;block<level->num_my_blocks;block++){
...
void mul_vectors(level_type * level, int id_c, double scale, int id_a, int id_b){
  ...
  if (level->use_offload) {
    device_mul_vectors(level, id_c, scale, id_a, id_b);
  }
  else {
  PRAGMA_THREAD_ACROSS_BLOCKS(level,block,level->num_my_blocks)
  for(block=0;block<level->num_my_blocks;block++){
...
void scale_vector(level_type * level, int id_c, double scale_a, int id_a){
  ...
  if (level->use_offload) {
    device_scale_vector(level, id_c, scale_a, id_a);
  }
  else {
  PRAGMA_THREAD_ACROSS_BLOCKS(level,block,level->num_my_blocks)
  for(block=0;block<level->num_my_blocks;block++){
...
# if defined(SPEC_OPENACC)
    copy_vector_to_host(level);
# elif defined(SPEC_OPENMP_TARGET)
    copy_vector_to_host_openmp(level);
# endif
  }
#endif
...
// note, only non ghost zone values are included in this calculation
double norm2(level_type * level, int id_a){ // implements the max norm
  double _timeStart = getTime();
  ...
  if (level->use_offload) {
    max_norm = device_max_abs(level, id_a);
  }
  else {
...
double mean(level_type * level, int id_a){
  ...
  if (level->use_offload) {
    sum_level = device_sum(level, id_a);
  }
  else {
...
void shift_vector(level_type * level, int id_c, int id_a, double shift_a){
  ...
  if (level->use_offload) {
    device_shift_vector(level, id_c, id_a, shift_a);
  }
  else {
  PRAGMA_THREAD_ACROSS_BLOCKS(level,block,level->num_my_blocks)
  for(block=0;block<level->num_my_blocks;block++){
...
double error(level_type * level, int id_a, int id_b){
  ...
  double   max =      norm2(level,VECTOR_TEMP);                return(max);   // max norm of error function
...
void color_vector(level_type * level, int id_a, int colors_in_each_dim, int icolor, int jcolor, int kcolor){
  ...
  if(level->use_offload) {
    device_color_vector(level, id_a, colors_in_each_dim, icolor, jcolor, kcolor);
  }
  else {
  PRAGMA_THREAD_ACROSS_BLOCKS(level,block,level->num_my_blocks)
  for(block=0;block<level->num_my_blocks;block++){

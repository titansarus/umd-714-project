...
// Create the levels (1-N) on the device
  for(level=1;level<all_grids->num_levels;level++){
     create_level_device(all_grids->levels[level]);
     build_exchange_ghosts_device(all_grids->levels[level]);
  }
// Build the restriction and interpolation all-levels
  for(level=0;level<all_grids->num_levels;level++){
     build_restriction_device(all_grids->levels[level]);
     build_interpolation_device(all_grids->levels[level]);
  }
// build the boxs for exchange_ghost, restriction, and interpolation 
  for(level=0;level<all_grids->num_levels;level++){
     build_box_device(all_grids->levels[level],1);
  }
#endif
...
// Create the levels (1-N) on the device
  for(level=1;level<all_grids->num_levels;level++){
     create_level_device_openmp(all_grids->levels[level]);
     build_exchange_ghosts_device_openmp(all_grids->levels[level]);
  }
// Build the restriction and interpolation all-levels
  for(level=0;level<all_grids->num_levels;level++){
     build_restriction_device_openmp(all_grids->levels[level]);
     build_interpolation_device_openmp(all_grids->levels[level]);
  }
// build the boxs for exchange_ghost, restriction, and interpolation 
  for(level=0;level<all_grids->num_levels;level++){
     build_box_device_openmp(all_grids->levels[level],1);
  }
#endif
  ...
  do_sync();  // synchronize GPU at the end of the setup phase
...
void richardson_error(mg_type *all_grids, int levelh, int u_id){
  ...
  double norm_of_u2h_minus_uh  = norm2(all_grids->levels[levelh+1],VECTOR_TEMP); // || u^2h - R u^h  ||max
  double norm_of_u4h_minus_u2h = norm2(all_grids->levels[levelh+2],VECTOR_TEMP); // || u^4h - R u^2h ||max
...
void MGSolve(mg_type *all_grids, int onLevel, int u_id, int F_id, double a, double b, double dtol, double rtol){
  ...
  if(dtol>0){
    ...
    norm_of_DinvF = norm2(all_grids->levels[onLevel],VECTOR_TEMP);		// ||D^{-1}F||
  }
  if(rtol>0)norm_of_F = norm2(all_grids->levels[onLevel],F_id);		// ||F||
  ...
  for(v=0;v<maxVCycles;v++){   
    ...
    double norm_of_residual = norm2(all_grids->levels[level],VECTOR_TEMP);
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef VERBOSE
  if(all_grids->levels[onLevel]->my_rank==0){fprintf(stdout,"done (%f seconds)\n",getTime()-MG_Start_Time);} // used to monitor variability in individual solve times
  #else
...
void FMGSolve(mg_type *all_grids, int onLevel, int u_id, int F_id, double a, double b, double dtol, double rtol){
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef VERBOSE
  if(all_grids->levels[onLevel]->my_rank==0){fprintf(stdout,"done (%f seconds)\n",getTime()-FMG_Start_Time);} // used to monitor variability in individual solve times
  #else
  if(all_grids->levels[onLevel]->my_rank==0){fprintf(stdout,"done\n");}
  #endif
}
...
void MGPCG(mg_type *all_grids, int onLevel, int x_id, int F_id, double a, double b, double dtol, double rtol){
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef VERBOSE
  double MGPCG_Start_Time = getTime();
  #endif
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  double norm_of_r0 = norm2(level,r_id);                                         // the norm of the initial residual...
  ...
  while( (j<jMax) && (!CGFailed) && (!CGConverged) ){                           // while(not done){
    ...
    //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  //double norm_of_r = norm2(level,r_id);                                        //   norm of intermediate residual (delusional convergence)
    residual(level,VECTOR_TEMP,x_id,F_id,a,b);                                  //   true residual
    double norm_of_r = norm2(level,VECTOR_TEMP);                                 //   norm of true residual (true convergence test)
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef VERBOSE
  if(all_grids->levels[onLevel]->my_rank==0){fprintf(stdout,"done (%f seconds)\n",getTime()-MGPCG_Start_Time);} // used to monitor variability in individual solve times
  #else
  if(all_grids->levels[onLevel]->my_rank==0){fprintf(stdout,"done\n");}
  #endif
}

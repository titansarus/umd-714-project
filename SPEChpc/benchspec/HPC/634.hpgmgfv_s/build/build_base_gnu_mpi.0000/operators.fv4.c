...
//------------------------------------------------------------------------------------------------------------------------------
  ...
  //#warning not threading norm2() calculations due to issue with XL/C, _Pragma, and reduction(max:bmax)
  ...
  #define PRAGMA_THREAD_ACROSS_BLOCKS(    level,b,nb     )    MyPragma(omp parallel for private(b) if(nb>1) schedule(static)                     )
  #define PRAGMA_THREAD_ACROSS_BLOCKS_SUM(level,b,nb,bsum)    MyPragma(omp parallel for private(b) if(nb>1) schedule(static) reduction(  +:bsum) )
  #define PRAGMA_THREAD_ACROSS_BLOCKS_MAX(level,b,nb,bmax)    MyPragma(omp parallel for private(b) if(nb>1) schedule(static) reduction(max:bmax) )
#else // older OpenMP versions don't support the max reduction clause
...
void print_level_stats(level_type *level, const char *str)
{
  double norms[100], means[100]; // 20 should be enough
  int vec, rank;
  do_sync();
  for (vec=0; vec<level->numVectors; ++vec) {
    means[vec] = mean(level, vec);
    norms[vec] = norm2(level, vec);
  }
#ifdef USE_MPI
  MPI_Allreduce(MPI_IN_PLACE, means, level->numVectors, MPI_DOUBLE, MPI_SUM, level->MPI_COMM_ALLREDUCE);
  MPI_Allreduce(MPI_IN_PLACE, norms, level->numVectors, MPI_DOUBLE, MPI_SUM, level->MPI_COMM_ALLREDUCE);
  MPI_Comm_rank(level->MPI_COMM_ALLREDUCE, &rank);
#else
  rank = 0;
#endif
  if (rank == 0) {
    if (str) printf("\n%s\n", str);
    for (vec=0; vec<level->numVectors; ++vec) {
      printf("vector=%d, norm=%lf, mean=%lf\n", vec, norms[vec], means[vec]);
    }
  }
  fflush(stdout);
  do_sync();

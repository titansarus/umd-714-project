...
!$acc data copyin(state_tmp,hy_dens_cell,hy_dens_theta_cell,hy_dens_int,hy_dens_theta_int,hy_pressure_int) &
!$acc   create(flux,tend,sendbuf_l,sendbuf_r,recvbuf_l,recvbuf_r) copy(state)
#elif defined(SPEC_OPENMP_TARGET)
!$omp target data map(to:state_tmp,hy_dens_cell,hy_dens_theta_cell,hy_dens_int,hy_dens_theta_int,hy_pressure_int) &
!$omp   map(alloc:flux,tend,sendbuf_l,sendbuf_r,recvbuf_l,recvbuf_r) map(tofrom:state)
#endif
...
#elif defined(SPEC_OPENMP_TARGET)
    !$omp target teams distribute parallel do collapse(2) private(stencil,vals,d3_vals,r,u,w,t,p)
#elif defined(SPEC_OPENMP)
    !$omp parallel do private(stencil,vals,d3_vals,r,u,w,t,p)
#endif
    do k = 1 , nz
#if defined(SPEC_OPENACC)
      !$acc loop vector private(stencil,vals,d3_vals,r,u,w,t,p)
#endif
      do i = 1 , nx+1
...
!$omp end target teams distribute parallel do
  ...
  subroutine compute_tendencies_z(state,flux,tend)
    ...
    !Compute fluxes in the x-direction for each cell
#if defined(SPEC_OPENACC)
    !$acc parallel loop gang vector collapse(2) private(stencil,vals,d3_vals,r,u,w,t,p) async(1)
#elif defined(SPEC_OPENMP_TARGET)
    !$omp target teams distribute parallel do collapse(2) private(stencil,vals,d3_vals,r,u,w,t,p)
#elif defined(SPEC_OPENMP)
    !$omp parallel do private(stencil,vals,d3_vals,r,u,w,t,p)
...
# elif defined(SPEC_OPENMP_TARGET
    !$omp target data use_device_ptr(recvbuf_l,recvbuf_r)
# endif
#endif
...
# elif defined(SPEC_OPENMP_TARGET)
    !$omp target data use_device_ptr(sendbuf_l,sendbuf_r)
# endif
#else  
# if defined(SPEC_OPENACC)
...
!$omp end target teams distribute parallel do
    ...
    if (data_spec_int == DATA_SPEC_INJECTION) then
      if (myrank == 0) then
#if defined(SPEC_OPENACC)
        !$acc parallel loop async(1) private(z)
#elif defined(SPEC_OPENMP_TARGET)
        !$omp target teams distribute parallel do private(z)
#elif defined(SPEC_OPENMP)
...
# endif
  ...
  include 'mpif.h' 
    ...
    if (etime == 0) then
      !Create the file
      call ncwrap( nf90mpi_create( MPI_COMM_WORLD , trim(outfile) , IOR(NF_NOCLOBBER,NF_64BIT_OFFSET) , MPI_INFO_NULL , ncid ) , __LINE__ )
      ...
      !Open the file
      call ncwrap( nfmpi_open( MPI_COMM_WORLD , trim(outfile) , nf90_write , MPI_INFO_NULL , ncid ) , __LINE__ )
    ...
    do k = 1 , nz
      do i = 1 , nx
        ...
        theta(i,k) = ( state(i,k,ID_RHOT) + hy_dens_theta_cell(k) ) / ( hy_dens_cell(k) + state(i,k,ID_DENS) ) &
	             - hy_dens_theta_cell(k) / hy_dens_cell(k)
      enddo
    enddo

    !Write the grid data to file with all the processes writing collectively
    st3=(/i_beg,k_beg,num_out+1/) 
    ct3=(/nx,nz,1/) 
    call ncwrap( nfmpi_put_vara_double_all( ncid ,  dens_varid , st3 , ct3 , dens  ) , __LINE__ )
    st3=(/i_beg,k_beg,num_out+1/)
    ct3=(/nx,nz,1/)
    call ncwrap( nfmpi_put_vara_double_all( ncid ,  uwnd_varid , st3 , ct3 , uwnd  ) , __LINE__ )
    st3=(/i_beg,k_beg,num_out+1/); ct3=(/nx,nz,1/)
    call ncwrap( nfmpi_put_vara_double_all( ncid ,  wwnd_varid , st3 , ct3 , wwnd  ) , __LINE__ )
    st3=(/i_beg,k_beg,num_out+1/); ct3=(/nx,nz,1/)
    call ncwrap( nfmpi_put_vara_double_all( ncid , theta_varid , st3 , ct3 , theta ) , __LINE__ )
    ...
    if (masterproc) then
      st1=(/num_out+1/); ct1=(/1/) 
      etimearr(1) = etime 
      call ncwrap( nfmpi_put_vara_double( ncid , t_varid , st1 , ct1 , etimearr ) , __LINE__ )
    endif
